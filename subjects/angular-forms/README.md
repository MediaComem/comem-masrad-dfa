...
<form
  `#greetingForm="ngForm"
  `
  (submit)="displayGreeting(`greetingForm`)"
></form>
```
...
```html
<ion-button `[disabled]="greetingForm.invalid" ` type="submit"
  >Submit</ion-button
>
```
...
```html
<p
  *ngIf="`greetingForm.get('greeting')`.hasError('required')
          && `greetingForm.get('greeting')`.dirty"
>
  Name is required
</p>
<p
  *ngIf="`greetingForm.get('greeting')`.hasError('notIn')
          && `greetingForm.get('greeting')`.dirty"
>
  Name is forbidden
</p>
