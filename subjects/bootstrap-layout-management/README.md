...
- `display`: you can force an inline component to behave like a block element, and _vice-versa_.
...
- `position`: you can force an element to a special position (_`fixed`, `absolute`, `relative`_), that can pull it out of the flow.
...
For example, we would like our _Lists_ title to span on all 12 columns...
...
Let's say that we want the _Icons_ section and the _Buttons_ section of the `index.html` to be **on the same row**, with both a **4-columns width**, but the former **aligned to the left**, and the latter **aligned to the right**.

We would need to offset our _Buttons_ section by **4 columns** to achieve that. And this can be done by using the `.col-md-offset-*`:
...
> Each screen-dependant class is activated when the screen has the correct widht **or is wider** (_unless you defined otherwise_).
...
Let's we want our _Tables_ section to be hidden on extra-small (`xs`) screens:
